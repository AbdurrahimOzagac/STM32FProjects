...
int8_t BMP180_Poll(BMP180_Handle_t *handle) {
    ...
    switch (handle->state) {
        ...
        case BMP180_STATE_WAIT_PRESS: {
            ...
            // Verileri anlamlı değerlere çevir
            ...
            // Yeni döngü için sıcaklık komutunu gönder
    ...
    // Switch-case'den beklenmedik bir şekilde çıkılırsa
    return BMP180_ERROR;
}
...
static int8_t BMP180_Read_Calibration(BMP180_Handle_t *handle) {
    ...
    // Tüm kalibrasyon verilerini tek seferde oku
...
static void BMP180_Get_Temperature_Celsius(uint16_t ut, BMP180_Calib_t *calib,
    ...
    // Basınç hesaplaması için gerekli
    *out_b5_ref = x1 + x2;
...
void BMP180_Set_Sea_Level_Pressure(BMP180_Handle_t *handle, float pressure_pa) {
    if (handle != NULL && pressure_pa > 1000.0f) { // Güvenlik kalkanı! (1000 Pa altı Dünya'da pek mümkün değil)
